...
test('ユーザーの新規登録/ログアウト/ログイン', async ({ page }) => {
  ...
  await expect(page.getByText('6文字以上で入力してください')).toHaveCount(2);
  await expect(
    page.getByText('半角英字(大・小)、数字、記号を含めてください'),
  ).toHaveCount(2);
  ...
  await expect(page.getByText('6文字以上で入力してください')).toBeVisible();
  await expect(
    page.getByText('半角英字(大・小)、数字、記号を含めてください'),
  ).toBeVisible();
  await expect(page).toHaveURL('/login');
  ...
  await page.getByRole('button', { name: 'メールアドレスでログイン' }).click();
  await expect(page.getByText('システムエラーが発生しました')).toBeVisible();
  await expect(page).toHaveURL('/login');
  // 10. 認証前）新規登録で作成済みのユーザー情報を入力してログインする
  await page.getByLabel('メールアドレス').fill('user1@example.com');
  await page.getByLabel('パスワード').click();
